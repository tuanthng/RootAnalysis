...
data_base0 = '/home/foz/Desktop/Tuan/'
data_base = data_base0 + '/151117 Col ht timelapse I flipped plate bent right way/'
...
original_images = data_base + 'Image_{}.bmp'
...
offsets = [ off ]
tip_locations = [ tp+np.array((0, -80)) ]
for i in range(1, Nf):
...
#    v_point = F[np.rint(tp[1]), np.rint(tp[0]), :]
    ...
    tip_locations.append(new_tp + np.array((0,-80)))
...
mid_Exx = nd.map_coordinates(Exx, (y_all+80, x_all))
mid_Exy = nd.map_coordinates(Exy, (y_all+80, x_all))
mid_Eyx = nd.map_coordinates(Eyx, (y_all+80, x_all))
mid_Eyy = nd.map_coordinates(Eyy, (y_all+80, x_all))
...
stretch_yy = nd.map_coordinates(Eyy, (y_all+80, x_all))
...
for i in range(1, Nf):
    ...
    v = nd.map_coordinates(F[:,:,0], (y_all+80, x_all))
